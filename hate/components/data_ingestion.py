...
class DataIngestion:
    ...
    def download_file(self):
        ...
        if not os.path.exists(self.data_ingestion_config.ZIP_FILE_PATH):
            try:
                response = request.urlopen(self.data_ingestion_config.source_url)
                content_type = response.info().get('Content-Type')
                if 'zip' in content_type:
                    with open(self.data_ingestion_config.ZIP_FILE_PATH, 'wb') as out_file:
            ...
            print('hi')
            # logger.info(f"File already exists of size: {get_size(Path(self.data_ingestion_config.ZIP_FILE_PATH))}")
